  ...
  newday:
    ...
    for(;;){
        ...
        if(!g_world.finished){
            ...
            if(g_world.h.carrying<0 && g_world.tick-lastStick>120*60*4 && mku(g_world.seed,"stick-say",g_world.tick)<0.02){ lastStick=g_world.tick; HerderUtterance u=herder_speak_kind(&g_world,EV_stick,4,0.2); if(u.ok){ snprintf(curline,sizeof(curline),"%s",u.text); lineUntil=frame+(int)(u.seconds*60); snprintf(lastHeard,sizeof(lastHeard),"%s",u.text); } }
        }
